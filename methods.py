...
def save_active_platforms(apnames, ap):
    for x in ap:
        ...
        png_names = []
        if os.path.isfile(x + "/logo.png"):
            png_names.append("logo")
        if os.path.isfile(x + "/run_icon.png"):
            png_names.append("run_icon")

        for name in png_names:
            with open(x + "/" + name + ".png", "rb") as pngf:
                data = pngf.read()

            png_str = " /* AUTOGENERATED FILE, DO NOT EDIT */ \n"
            png_str += " static const unsigned char _" + x[9:] + "_" + name + "_png[] = {"
            png_str += ",".join([str(b) for b in data])
            png_str += "};\n"

            # NOTE: It is safe to generate this file here, since this is still executed serially
            wf = x + "/" + name + "_png.gen.h"
